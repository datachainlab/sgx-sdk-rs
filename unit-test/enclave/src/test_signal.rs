...
pub fn test_cpuid_trap() -> TestResult {
    ...
    #[cfg(sgx_sim)]
    {
        // In simulation mode, CPUID should be patched to INT3 and handled by our trap handler
        let cpuid_result = unsafe {
            use core::arch::x86_64::__cpuid;
            __cpuid(0)
        };

        // If we reach here, the trap handler successfully handled the INT3
        // and execution continued
        println!("CPUID trap test: Successfully trapped and continued execution");
        println!(
            "CPUID result: eax={:x}, ebx={:x}, ecx={:x}, edx={:x}",
